...
type Cell = Array<Placeholder | Task>;
...
export default function KanbanBoard() {
  ...
  const getTasksByStatus = (tasks: any[], statusId: number) => {
  ...
  const makeTaskGrid = (tasks: Task[], status_id: number): Grid => {
    let grid: Grid = [];

    const sorted_tasks = getSortedTasks(getTasksByStatus(tasks, status_id));
    const items_present = sorted_tasks.length !== 0;

    if (!items_present) {
      return [];
    }

    const first = sorted_tasks[0];
    grid.push([ { is_placeholder: true, priority: first.priority - 1, order: 0 }, ]);
    grid.push([
      { is_placeholder: true, priority: first.priority, order: first.order - 1 },
      first,
    ]);
    let row_i = grid.length - 1;

    for (let i = 1; i < sorted_tasks.length - 1; ++i) {
      const prev = sorted_tasks[i];
      const curr = sorted_tasks[i + 1];
      if (prev.priority != curr.priority) {
        // pad the old row
        grid[row_i].push({ is_placeholder: true, priority: curr.priority, order: prev.order + 1 });
        // determine the padding between in-col cells
        if (curr.priority - prev.priority === 2) {
          grid.push([ { is_placeholder: true, priority: curr.priority - 1, order: 0 } ]);
        } else if (curr.priority - prev.priority > 2) {
          grid.push([ { is_placeholder: true, priority: prev.priority + 1, order: 0 } ]);
          grid.push([ { is_placeholder: true, priority: curr.priority - 1, order: 0 } ]);
        }
        // pad a new row
        grid.push([
          { is_placeholder: true, priority: curr.priority, order: curr.order - 1 },
          curr,
        ]);
        row_i = grid.length - 1;
      } else {
        // determine the padding between in-row cells
        if (curr.order - prev.order >= 2) {
          grid[row_i].push({ is_placeholder: true, priority: curr.priority, order: prev.order + 1 });
          if (curr.order - prev.order > 2) {
            grid[row_i].push({ is_placeholder: true, priority: curr.priority, order: curr.order - 1 });
          }
        }
        // add a new col after padding
        grid[row_i].push(curr);
      }
    }

    const last = sorted_tasks[sorted_tasks.length - 1];
    // close off the last in-row padding process
    grid[row_i].push({ is_placeholder: true, priority: last.priority, order: last.order + 1 });
    // close off the last in-col padding process
    grid.push([ { is_placeholder: true, priority: last.priority + 1, order: 0 } ]);

    return grid;
  };

  const getTaskPrioritiesWithPlaceholders = (tasks: Task[], statusId: number) => {
    const statusTasks = getTasksByStatus(tasks, statusId);
    if (statusTasks.length === 0) return [0];

    const priorities = [...new Set(statusTasks.map(t => t.priority))].sort((a, b) => a - b);

    const withPlaceholders: number[] = [];
  ...
  const getAllOrdersWithPlaceholders = (tasks: Task[], statusId: number, priority: number) => {
    const grid = makeTaskGrid(tasks, statusId);
  ...
  const renderGrid = (statusId: number) => {
    const grid = makeTaskGrid(tasks, statusId);

    const priorities = getTaskPrioritiesWithPlaceholders(tasks, statusId);
    ...
    return (
      <div className="space-y-2">
        {priorities.map(priority => {
          const orders = getAllOrdersWithPlaceholders(tasks,statusId, priority);
