            ...
            <Droppable droppableId={String(status.id)} key={status.id}>
              {(provided, snapshot) => (
                <div ref={provided.innerRef} {...provided.droppableProps} className="min-w-[300px]">
                  <div
                    className={
                      "rounded-lg border border-foreground/10 bg-background p-3 shadow-sm transition-colors" +
                      (snapshot.isDraggingOver ? " outline outline-2 outline-dashed outline-foreground/30 bg-foreground/5" : "")
                    }
                  >
                    <h4 className="text-base font-medium">{status.name}</h4>
                    <div className="mt-2 max-h-[70vh] overflow-y-auto min-h-[180px]">
                        ...
                        <Draggable draggableId={String(t.id)} index={idx} key={t.id}>
                          {(prov, dragSnapshot) => (
                            <div
                              ...
                              className={
                                "mt-2 rounded-md bg-white p-2 shadow-sm dark:bg-zinc-900 transition-transform" +
